...
from definitions import Algorithm, Graph, Route, RouteStop, MAX_RANGE, SPAlgorithm
    ...
    def getShortestPath(
        ...
        while minPriorityQ:
            ...
            for edge in graph.getMunicipalityEdges(currentMuni):
                ...
                hasSupercharger: bool = graph.getMunicipalityHasSupercharger(
                    neighborMuni
                )
        ...
        route: Route = Route(stops=[], algorithm=SPAlgorithm.DIJKSTRA)
        ...
        for i in range(len(route.stops)):
            route.stops[i].distance = (
                distances[route.stops[i].muniCode] - currentDistance
            )
